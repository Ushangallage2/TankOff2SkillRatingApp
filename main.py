...
print(f"DB_HOST: {DB_HOST}, DB_USERNAME: {DB_USERNAME}, DB_NAME: {DB_NAME}, DB_PORT: {port}")  # Debug print
...
class SkillRatingApp(MDApp):
    ...
    def create_db(self):
        try:
            ...
            self.conn = mysql.connector.connect(
                host='mysql-3e2a5f7c-furythegreatest-f175.b.aivencloud.com',
                user='avnadmin',
                password='AVNS_EGmu-kz_7_Q_tFrFzdX',
                database='defaultdb',
                port=16308,
                ssl_disabled=False
            )
        ...
        try:
            ...
            cursor.execute(''' 
                   SELECT p.player_name, 
                          ...
                          COALESCE(c.country_flag, 'flags/default.png') AS country_flag  
            ...
            for idx, row in enumerate(results):
                ...
                medal_path = resource_path([
                                               'positions/1st-prize.png',
                                               'positions/2nd-place.png',
                                               'positions/3rd-place.png'
                                           ][idx])
    ...
    def build(self):
        ...
        print("Main layout built successfully.")
     ...
     #   style_button(self.performance_leaderboard_button)
      ...
      # style_button(self.stats_button)
        ...
        if video_id and thumbnail_path:
            ...
            self.video_player = YouTubeVideoPlayer("aVuUwRy7HUE", fallback_thumbnail,self.music_control_button)
        ...
        self.refresh_button = Button(
            background_normal=resource_path('positions/refresh.png'),  # Path to your home icon image
            size_hint=(None, None),
            size=(50, 50),
    ...
    def fetch_player_stats(self, player_name):
        ...
        try:
            ...
            if result and result[0] > 0:
                ...
                resolved_flag_path = resource_find(country_flag)
                if not resolved_flag_path:
                    resolved_flag_path = resource_find('flags/default.png')
                    if not resolved_flag_path:
                        print("Default flag not found!")
                ...
                for stat_name, stat_value in stats_message:
                    ...
                    # Now load the image using the resolved path
                    flag_image = Image(
                        source=resolved_flag_path,
                        size_hint=(None, None),
                        size=(50, 50)
                    )

              #  self.main_layout.add_widget(flag_image)
                ...
                def update_player_flag(selected_flag):
                    ...
                    self.update_flag_in_database(player_name, selected_flag)
                    flag_image.source = selected_flag
                    flag_image.reload()
    ...
    def add_player_inputs(self, team_name, player_identifier):
        ...
        dropdown_button = Button(
            ...
            background_normal=resource_find("positions/dropdown.png"),  # replace with your icon path
            background_down=resource_find("positions/dropdown.png"),  # optional: same image when pressed
            text="",  # no text
        )
        ...
        dynamic_rows = []
        add_rows_button = Button(text=f"insert {player_identifier} stats before he switched if he switched!",
    ...
    def calculate_skill_rating_logic(self, player, outcome, teammates_average_rating, opponents_average_rating,teammates_avg_level,
    opponents_avg_level):

        # Initialize skill rating
        skill_rating = float(player.skill_rating) if player.skill_rating is not None else 0.0

        # Convert averages to floats with validation
        try:
            teammates_avg = float(teammates_average_rating)
            opponents_avg = float(opponents_average_rating)
        ...
        if player.time_played is not None and player.time_played > 0:
            performance_score = (
        ...
        if outcome == 'win':
            ...
            performance_score = (
                    (flags_weight * player.flags_captured +
                     kills_weight * player.kills +
                     (xp_weight * real_xp) / 1000) * (480 / (60 * player.time_played))
            )
        ...
        # === Include player's level as a small positive influence ===
        level_weight = (-0.8)
        max_level = 236
        try:
        ...
        # Normalize gap: cap at ±50 for max scaling
        clamped_gap = max(min(level_gap, 50), -50)

        # Each 10-level gap adds or subtracts ~0.1 (scaled)
        level_gap_modifier = -(clamped_gap / 10) * 0.1  # Negative gap gives a bonus, positive gives penalty

        skill_rating += level_gap_modifier

        return round(skill_rating/100, 2)

    def calculate_skill_rating(self, instance):  # Add self
        player_stats = {}
        all_players = set()
        ...
        for player in self.players:
            ...
            if not self.player_exists_in_database(name):
                new_players_to_create.append(name)
            else:
        ...
        if new_players_to_create:
            self.confirm_create_players(new_players_to_create)
            return

        print("All players are in the database.")
        self.submit_button.disabled = False
        ...
        for player in self.players:
            ...
            try:
                kills = int(player['inputs'][0].text or 0)
                flags_captured = int(player['inputs'][1].text or 0)
                level = int(player['inputs'][2].text or 0)
                time_played = float(player['inputs'][3].text or 0)
                xp = int(player['inputs'][4].text or 0)
            except ValueError:
            ...
            for i in range(0, len(dynamic_inputs), 2):
                try:
                    switched_kills = int(dynamic_inputs[i].text or 0)
                    switched_flags = int(dynamic_inputs[i + 1].text or 0) if i + 1 < len(dynamic_inputs) else 0

                    kills += switched_kills
                    flags_captured += switched_flags

                    opposing_team = 'Blue' if team_name == 'Red' else 'Red'
                    team_stats[opposing_team]['kills'] += switched_kills
                    team_stats[opposing_team]['flags'] += switched_flags
                except ValueError:
                    continue
            ...
            player_instance = Player(name, level, kills, flags_captured, time_played, xp, skill_rating)
        ...
        red_ratings = [self.fetch_player_rating(p['name'].text.strip()) for p in self.players if p['team'] == 'Red']
        blue_ratings = [self.fetch_player_rating(p['name'].text.strip()) for p in self.players if
                        p['team'] == 'Blue']
        ...
        # Final loop to calculate updated skill ratings
        for name, stats in player_stats.items():
            player_team = stats['team']
            opposing_team = 'Blue' if player_team == 'Red' else 'Red'

            outcome = 'draw' if winning_team == 'Draw' else ('win' if player_team == winning_team else 'lose')

            teammates = red_ratings if player_team == 'Red' else blue_ratings
            opponents = blue_ratings if player_team == 'Red' else red_ratings

            avg_teammate_rating = sum(teammates) / len(teammates) if teammates else 0
            avg_opponent_rating = sum(opponents) / len(opponents) if opponents else 0

            avg_teammate_level = sum(team_levels[player_team]) / len(team_levels[player_team]) if team_levels[
                player_team] else 0
            avg_opponent_level = sum(team_levels[opposing_team]) / len(team_levels[opposing_team]) if team_levels[
                opposing_team] else 0

            player_instance = Player(name, stats['level'], stats['kills'], stats['flags_captured'],
                                     stats['time_played'], stats['xp'], stats['skill_rating'])

            skill_rating = self.calculate_skill_rating_logic(
                player_instance,
                outcome,
                avg_teammate_rating,
                avg_opponent_rating,
            ...
            stats['skill_rating'] = skill_rating
            print(skill_rating)
    ...
    def confirm_create_players(self, player_names):
        ...
        popup = Popup(title="Player Creation Confirmation", content=content, size_hint=(0.6, 0.4))
        popup.open()
    ...
    def insert_new_player_into_database(self, player_name):
        ...
        try:
            ...
            # Clean up the match if inserted and player insert failed
            if match_inserted:
                try:
                    cursor.execute('DELETE FROM matches WHERE match_number = %s', (match_number,))
                    self.conn.commit()
                except Exception as cleanup_error:
                    print(f"Cleanup error after player insert: {cleanup_error}")
    ...
    def fetch_player_rating(self, player_name):
        try:
            cursor = self.conn.cursor()
            cursor.execute(
                'SELECT player_total_rating FROM overall_player_skill_rating WHERE player_name = %s',
                (player_name,)
            )
            result = cursor.fetchone()
            cursor.close()
    ...
    def perform_export(self, *args):
        ...
        global cursor
        match_inserted = False  # Track if we inserted match
        ...
        try:
            ...
            # Delete oldest match sets if more than 5
            cursor.execute('''
                SELECT DISTINCT match_number
                FROM player_skill_rating
                ORDER BY match_number DESC
            ''')
            match_numbers = [row[0] for row in cursor.fetchall()]

            if len(match_numbers) >= 5 and match_number not in match_numbers:
                to_delete = match_numbers[5:]
                for old_match in to_delete:
                    cursor.execute('DELETE FROM player_skill_rating WHERE match_number = %s', (old_match,))

            # Insert match — may be removed if export fails later
            cursor.execute('''
                INSERT INTO matches (match_number, match_date) 
                VALUES (%s, CURDATE())
                ON DUPLICATE KEY UPDATE match_date = CURDATE()
            ''', (match_number,))
            match_inserted = True  #  Mark that we inserted

            for player in self.players:
                player_name = player['name'].text.strip()
                if not player_name:
                    raise ValueError("Player name is required.")

                kills = int(player['inputs'][0].text or 0)
                flags = int(player['inputs'][1].text or 0)
                xp = int(player['inputs'][4].text or 0)
                skill_rating = self.player_stats.get(player_name, {}).get('skill_rating')

                if skill_rating is None:
                    raise ValueError(f"Skill rating not calculated for player '{player_name}'.")

                # Insert one row per player+match
                cursor.execute('''
                    INSERT INTO player_skill_rating 
                    (player_name, kills, flags_captured, xp, skill_rating, match_number)
                    VALUES (%s, %s, %s, %s, %s, %s)
                ...
                ''', (player_name, kills, flags, xp, skill_rating, match_number))

                # Update overall stats
                cursor.execute('''
                    INSERT INTO overall_player_skill_rating 
                    (player_name, total_kills, total_flags, total_xp, total_matches)
                    VALUES (%s, %s, %s, %s, 1)
                    ON DUPLICATE KEY UPDATE
                        total_kills = total_kills + VALUES(total_kills),
                        total_flags = total_flags + VALUES(total_flags),
                        total_xp = total_xp + VALUES(total_xp),
                        total_matches = total_matches + 1,
                        KPG = (total_kills + VALUES(total_kills)) / (total_matches + 1),
                        FPG = (total_flags + VALUES(total_flags)) / (total_matches + 1),
                        XPG = (total_xp + VALUES(total_xp)) / (total_matches + 1)
                ''', (player_name, kills, flags, xp))
            ...
            # Cleanup: remove match if we inserted it
            if match_inserted:
                try:
                    cursor.execute('DELETE FROM matches WHERE match_number = %s', (match_number,))
                    self.conn.commit()
        ...
        try:
            ...
            cursor.execute('''
                SELECT
                    ...
                    COALESCE(c.country_flag, 'flags/default.png') AS flag
                FROM player_skill_rating p
                LEFT JOIN player_country_flags c ON p.player_name = c.player_name
                GROUP BY p.player_name
            ...
            for idx, row in enumerate(results):
                ...
                leaderboard_grid.add_widget(
                    Label(text=f"{row[4]:.2f}", size_hint_y=None, height=40))  # Ovelogicrall Skill

                # Add flag image on this line
            flag_image_path = row[5] if isinstance(row[5], str) else resource_path(
                'flags/default.png')  # Flag path
            flag_image = Image(source=resource_path(flag_image_path), size_hint=(0.6, None), size=(30, 30))
